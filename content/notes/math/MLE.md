...
The $\frac{1}{N}$ factor normalizes the loss so it does not scale with dataset size, making it a [Monte Carlo](https://en.wikipedia.org/wiki/Monte_Carlo_method) estimate of the population objective:

$$
\mathcal{L}_{\text{MLE}}(\theta) = -\mathbb{E}_{\mathbf{x} \sim p_{data}(\mathbf{x})}\left[\log p_\theta(\mathbf{x})\right] \tag{5}
$$
...
&\Rightarrow \hat{p} = \frac{H}{N} \tag{9}
\end{align}
$$
...
$$
p_\theta(x) = \frac{1}{\sqrt{2\pi\sigma^2}} \exp\!\left(-\frac{(x-\mu)^2}{2\sigma^2}\right) \tag{10}
$$

Taking the log, the log-likelihood of a single sample is:

$$
\log p_\theta(x) = -\frac{1}{2}\log(2\pi\sigma^2) - \frac{(x - \mu)^2}{2\sigma^2} \tag{11}
$$

Summing over the dataset, the total log-likelihood is:

$$
\ell(\mu, \sigma^2) = \sum_{i=1}^{N} \log p_\theta(x^{(i)}) = -\frac{N}{2}\log(2\pi\sigma^2) - \frac{1}{2\sigma^2}\sum_{i=1}^{N}(x^{(i)} - \mu)^2 \tag{12}
$$
...
&\Rightarrow \hat{\mu} = \frac{1}{N}\sum_{i=1}^{N} x^{(i)} \tag{13}
\end{align}
$$
...
&\Rightarrow \hat{\sigma}^2 = \frac{1}{N}\sum_{i=1}^{N}(x^{(i)} - \hat{\mu})^2 \tag{14}
\end{align}
$$
...
$$
y^{(i)} = \mathbf{w}^\top \mathbf{x}^{(i)} + \varepsilon^{(i)}, \quad \varepsilon^{(i)} \overset{\text{i.i.d.}}{\sim} \mathcal{N}(0, \sigma^2) \tag{15}
$$

This means the conditional distribution of $y^{(i)}$ given $\mathbf{x}^{(i)}$ is:

$$
p_\mathbf{w}(y^{(i)} \mid \mathbf{x}^{(i)}) = \mathcal{N}(y^{(i)};\, \mathbf{w}^\top \mathbf{x}^{(i)},\, \sigma^2) = \frac{1}{\sqrt{2\pi\sigma^2}} \exp\!\left(-\frac{(y^{(i)} - \mathbf{w}^\top \mathbf{x}^{(i)})^2}{2\sigma^2}\right) \tag{16}
$$

The log-likelihood of a single pair is:

$$
\log p_\mathbf{w}(y^{(i)} \mid \mathbf{x}^{(i)}) = -\frac{1}{2}\log(2\pi\sigma^2) - \frac{(y^{(i)} - \mathbf{w}^\top \mathbf{x}^{(i)})^2}{2\sigma^2} \tag{17}
$$

Summing over the dataset and dropping the constant term (which does not depend on $\mathbf{w}$), the total log-likelihood is:

$$
\ell(\mathbf{w}) = -\frac{1}{2\sigma^2} \sum_{i=1}^{N} \left(y^{(i)} - \mathbf{w}^\top \mathbf{x}^{(i)}\right)^2 \tag{18}
$$

Maximizing $\ell(\mathbf{w})$ over $\mathbf{w}$ is equivalent to minimizing the **sum of squared residuals**:

$$
\hat{\mathbf{w}}_{\text{MLE}} = \underset{\mathbf{w}}{\arg\min} \sum_{i=1}^{N} \left(y^{(i)} - \mathbf{w}^\top \mathbf{x}^{(i)}\right)^2 \tag{19}
$$
...
&\Rightarrow \hat{\mathbf{w}} = (\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{y} \tag{20}
\end{align}
$$
...
$$
\begin{align}
\mathcal{D}_{KL}(p_{data}(\mathbf{x}) \| p_\theta(\mathbf{x})) &= \mathbb{E}_{\mathbf{x} \sim p_{data}(\mathbf{x})}\left[\log \frac{p_{data}(\mathbf{x})}{p_\theta(\mathbf{x})}\right] \notag \\
&= -\mathbb{E}_{\mathbf{x} \sim p_{data}(\mathbf{x})}\left[\log p_\theta(\mathbf{x})\right] + \underbrace{\mathbb{E}_{\mathbf{x} \sim p_{data}(\mathbf{x})}\left[\log p_{data}(\mathbf{x})\right]}_{\text{constant w.r.t. } \theta} \tag{21}
\end{align}
$$
